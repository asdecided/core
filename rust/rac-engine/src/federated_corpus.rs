...
use crate::graph_federated_corpus::{
    compose_verified_federation, GraphFederatedCorpusError, VerifiedGraphCorpus,
};
...
/// Load the retained version-2 graph model for operator and diagnostic
/// surfaces which must expose verified topology or resolution provenance.
///
/// This is the same verification/composition path used by ordinary reads;
/// callers cannot accidentally construct a second graph overlay. A
/// version-1 or unconfigured repository deliberately returns `Ok(None)`.
