...
```bash
docker pull ghcr.io/asdecided/core:v0.29.0
...
rac-gate:
  image:
    name: ghcr.io/asdecided/core:v0.29.0
    entrypoint: [""]
  script:
...
pipelines:
  pull-requests:
    '**':
      - step:
          name: decided gate
          image: ghcr.io/asdecided/core:v0.29.0
          script:
            - decided gate decisions/
```
...
```groovy
pipeline {
  agent { docker { image 'ghcr.io/asdecided/core:v0.29.0' } }
  stages {
    stage('decided gate') {
