...
- [REQ-001] The engine MUST construct one source-aware effective corpus from the verified local and inherited layers before deriving validation, resolution, search, graph, scope, export, cache, freshness, or MCP models. Consumers MUST NOT merge corpus walks independently and MUST NOT introduce a second resolver. The local walk MUST exclude the declared parent materialisation subtree so the same Markdown file cannot enter both layers.
- [REQ-002] Corpus items, validation rows, index entries, relationship endpoints, resolved artifacts, derived generations, and persistent-store records MUST carry enough source and layer identity to distinguish `(source, relative path)` and `(source, canonical id)`.
- [REQ-003] Existing unqualified references MUST resolve when exactly one artifact across both layers matches. The syntax `alias::canonical-id` MUST resolve only within the named source; aliases after `::` MUST be rejected. No layer may receive implicit resolution precedence.
- [REQ-004] A parent/child canonical-id collision MUST produce a deterministic cross-corpus error. A colliding legacy or title alias MAY coexist, but an unqualified reference to it MUST be ambiguous and require qualification.
- [REQ-005] Overrides MUST be declared under `## overrides` in `.decided/corpus.md` and MUST map one qualified parent canonical id to one local same-type replacement plus one live local Decision as rationale. An absent, ambiguous, cross-type, retired-rationale, or parent-to-parent mapping MUST be a validation error.
- [REQ-006] A valid override MUST make the local replacement effective for unqualified resolution, live retrieval, scope routing, and enforcement while leaving the original parent addressable by qualified id. Exports and responses MUST preserve the parent, its overridden state, and the complete override provenance.
- [REQ-007] Provenance MUST be preserved in resolution results, search and grounding results, relationship and validation findings, code-enforcement reports, MCP responses and audit records, and all export projections. At minimum it MUST distinguish local/inherited and name the stable source; inherited output MUST also identify the verified pin.
- [REQ-008] Inherited live Decisions MUST participate in `decisions-for`, `retrieve_grounding`, the MCP path lookup, `gate`, `sentry`, and `gate --code` through the existing scope and enforcement evaluators. Their `## Applies To` declarations and code-constraint globs MUST match against the child repository code tree, never the parent materialisation.
- [REQ-009] Enforcement and MCP MUST NOT expose a local-only bypass for a configured federated corpus. Human diagnostic reads and exports MAY accept `--local-only`; mutation commands MUST always receive only the local writable layer.
- [REQ-010] Local and inherited artifacts MUST share one deterministic lexical and relationship index. Ranking MUST add no source boost or local-first quota, MUST retain the v0.28 lexical graph-floor gate, and MUST use `(source, relative path)` as the stable final tie order in a federated corpus.
- [REQ-011] The six-tool MCP surface MUST remain unchanged. Existing id arguments MUST accept qualified ids, and responses MUST retain source/layer/pin provenance under the existing hard character budget; provenance MUST NOT be truncated before artifact content.
