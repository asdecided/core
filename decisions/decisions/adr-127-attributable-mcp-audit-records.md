...
drifted from the accepted audit contract in three ways. Its public attribution
carrier was not consistently named across the engine and documentation; it only
extracted top-level and a subset of collection identities; and it emitted bare
strings even though ADR-084 requires each returned identity to carry resolution
state and a provenance reference. Audit activation was also silent beyond the
generic HTTP listening message.
...
The canonical shared-server attribution carrier is `X-AsDecided-Principal`, as
fixed by ADR-098 and this public cutover. The Rust server accepts exactly one
carrier:
...
- duplicate occurrences are rejected with HTTP 400 and JSON-RPC error `-32023`;
- malformed values are rejected with HTTP 400 and JSON-RPC error `-32023`.
...
This decision fixes the public AsDecided carrier and supersedes any alternate
implementation spelling. ADR-084 and ADR-098 remain the governing audit and
shared-HTTP decisions.
...
- Proxies send one explicit AsDecided header; ambiguous or malformed attribution
  fails closed before a tool call runs.
...
### Accept more than one attribution carrier

Rejected. Multiple spellings make proxy configuration and audit review
ambiguous, and they would keep the public contract out of sync.
...
rules:
  - id: mcp-audit-uses-canonical-principal
    ...
    pattern: 'CANONICAL_PRINCIPAL_HEADER: &str = "X-AsDecided-Principal"'
    message: "Shared MCP audit attribution must keep X-AsDecided-Principal canonical."
