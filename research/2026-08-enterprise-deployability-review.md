...
## 11. Addendum — re-verification against `16d11d3` (#430/#431, 2026-08-04)

Three commits landed since the §10 re-verification: PR #430 "close native
truth and guidance drift" (~64 files), PR #431 "verify native releases before
publishing," and a direct follow-up commit (`16d11d3`) fixing `.dockerignore`
so the license notices reach the image build context. The chair re-verified
every touched finding against source, including reading both new ADRs and the
full release-workflow diffs. This round moves real weight:

**Resolved — 5** (was 0):

- **DEP-01 · Release verification pack.** `native-publish.yml` now runs the
  full battery first, then: archives include `LICENSE`/`NOTICE`/`THIRD-PARTY-
  NOTICES` (generated by the new `scripts/generate-third-party-notices.sh`);
  every archive gets a GitHub build-provenance attestation; an `evidence` job
  publishes `SHA256SUMS` and a CycloneDX SBOM (both themselves attested); all
  four image tags are cosign-signed via OIDC; the image carries the notices at
  `/usr/share/doc/asdecided/`; and `docs/security.md` gained a "Release
  verification" section with the exact `sha256sum -c` / `gh attestation
  verify` commands. First exercised at the next tagged release — worth
  confirming the assets appear then. (Residue routed to DEP-21: the new
  third-party actions are tag-pinned.)
- **DEP-02 · Releases gated on the battery.** `rust-spike.yml` gained a
  `workflow_call` trigger with a `ref` input, and all three publish lanes —
  native, crates.io, MCP registry — now `needs:` the full battery at the
  release tag before building or publishing anything. ADR-027 rule 2 is
  restored on the native path, as recommended.
- **DEP-03 · Agent-guidance drift.** `CLAUDE.md` imports now resolve
  (`decisions/prompts/`), the managed settled-decisions block is regenerated
  and current (including ADR-121 and the two new ADRs), `.mcp.json` runs
  `decided-mcp` under the `asdecided` key, and the session-start and
  release-gate prompts describe the Rust engine (`cargo test`, `decided
  validate decisions/`). The drift gate went repo-wide:
  `docs_contract.rs` now bans retired surfaces (`rac/prompts`,
  `rac/roadmaps`, retired CLI strings, the `lore` key) across docs, recipes,
  `.mcp.json`, and the generated agent-rules files — running on Linux,
  Windows, and macOS. (`.agent-context/` scratch remains — that is DEP-24.)
- **DEP-04 · Telemetry truth.** The compiled-in PostHog key is now the empty
  string, and **ADR-131 "Native Telemetry Is Local-Only"** (Accepted) records
  the amendment the council asked for — no sender, no ping, no network side
  channel, retained consent state as local compatibility data, and a clause
  that future collection needs a new decision and "must not be restored by
  reintroducing the retired key or by changing documentation alone." CLI
  strings, `docs/mcp.md`, `docs/security.md`, and `docs/index.md` all now
  state the no-sender reality. This closes the finding, including the CLI-side
  over-claim §10 flagged.
- **DEP-09 · Dead commands / tool counts / server key.** All recipes swept to
  the `asdecided` key and six tools (including the demo's verification steps);
  `docs/quickstart.md` pins `v0.26.2` with a digest-pinning example;
  `docs/index.md`'s dogfood link targets `decisions/`; `docs/ecosystem.md`
  rows updated — and the sweep is held by the new CI gate. **ADR-132
  "AsDecided MCP Server Identity"** (Accepted) settles the naming question
  with-the-grain of ADR-036/039, exactly as recommended. Residue: engine
  error strings still suggest the unimplemented `decided ingest`
  (`commands.rs:606,720`), and `docs/cli.md`'s `ingest`/`explorer` sections
  are DEP-16's.

**Substantially advanced — 3:**

- **DEP-08:** the docs-homepage license label now reads Apache-2.0 and the
  `tree/main/decisions` link is fixed — both CI-asserted. Open: the Accepted
  landing-page requirement still mandates `pip install requirements-as-code`
  and an "MIT license" footer (`decisions/requirements/rac-docs-site-landing-
  page.md:30,36`) — the corpus-side supersession is DEP-14 work.
- **DEP-14:** two clean amendment ADRs landed (131, 132), the scaffold no
  longer emits `lore`/`lore-org`, and the generated settled-decisions text
  says "AsDecided MCP tools." Open: ADR-085 ratification, ADR-075's job list,
  the `rust-spike` workflow name, stale Accepted artifacts, and the v0.26.x
  version-fence collision.
- **DEP-21:** `rust-spike.yml` now has `permissions: contents: read` — the
  missing-permissions gap is closed. Open, and now larger: SHA-pinning —
  the release lane gained more tag-pinned third-party actions
  (`anchore/sbom-action@v0`, `attest-build-provenance@v2`,
  `cosign-installer@v3`), and rust-spike still uses `checkout@v4`.

**Unchanged — 18:** DEP-05 (root `SECURITY.md` untouched — still no
acknowledgment SLA, supported-versions, or CVE commitment), DEP-06 (no LLM01
guidance in `docs/mcp.md`), DEP-07 (no non-loopback interlock), DEP-10 (no
`dependabot.yml`, no `deny.toml` — noting the no-egress claim could now be
CI-enforced via a banned-crates rule), DEP-11, DEP-12 (`http.rs` untouched),
DEP-13 (the three Actions still compile from source in consumer CI — now
unlockable since DEP-01's checksummed assets will exist), DEP-15, DEP-16
(`docs/cli.md` still opens "RAC ships a single command, `rac`, with
twenty-two subcommands"), DEP-17's remaining pack (no profile `audit:`
stanza, rotation, SIEM recipe, or data sheet), DEP-18, DEP-19, DEP-20
(CHANGELOG still missing v0.24.x–v0.26.0), DEP-23, DEP-24, DEP-25, DEP-26,
DEP-27, DEP-28, DEP-29.

Running tally across both rounds: **5 resolved, 3 substantially advanced,
3 partially addressed (DEP-16/17/22), 18 unchanged.** Tier 1 is roughly half
cleared; what remains of it is the policy-and-guardrail cluster — DEP-05
(vulnerability program), DEP-06 (LLM01 guidance), DEP-07 (bind interlock),
DEP-10 (dependency automation), DEP-11 (sustainability statement) — every one
of them days-not-weeks, and DEP-13 is now cheap to start since its
prerequisite shipped.

Two process observations from this round, offered neutrally: the
`.dockerignore` fix (`16d11d3`) landed as a direct commit to `main` with no
pull request, which sits awkwardly next to ADR-075's required pre-merge gate —
worth checking branch protection actually enforces the gate; and none of the
three commits carries a `CHANGELOG.md` entry yet, so the DEP-20 release-gate
check remains the backstop to watch at the next tag.
